        ...
        .run(['$rootScope', '$http', '$timeout', '$location', '$cookies', '$modal', '$translate', '$window',
            function($rootScope, $http, $timeout, $location, $cookies, $modal, $translate, $window) {
        		...
        		$rootScope.loadServerConfig = function(callbackAfterLoad) {
                    $http({method: 'GET', url: FLOWABLE.CONFIG.contextRoot+'/app/rest/server-configs'}).
        		...
        		$http.get(FLOWABLE.CONFIG.contextRoot+'/v1/account')
	        	...
	        	$rootScope.loadProcessDefinitionsCache = function() {
                    var promise = $http({
                        method: 'GET',
                        url: FLOWABLE.CONFIG.contextRoot+'/app/rest/admin/process-definitions?size=100000000'
                    }).success(function (data, status, headers, config) {
                        return data;
                    }).error(function (data, status, headers, config) {
                        return {'status': false};
	            ...
	            $rootScope.loadCaseDefinitionsCache = function() {
                    var promise = $http({
                        method: 'GET',
                        url: FLOWABLE.CONFIG.contextRoot+'/app/rest/admin/case-definitions?size=100000000'
                    }).success(function (data, status, headers, config) {
                        return data;
                    }).error(function (data, status, headers, config) {
                        return {'status': false};
