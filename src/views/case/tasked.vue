<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input style="width: 200px;" class="filter-item" :placeholder="$t('case.clientName')"
                v-model="listQuery.clientName">
      </el-input>
      <el-select clearable class="filter-item" style="width: 130px" v-model="listQuery.entityType"
                 :placeholder="$t('case.entityType')">
      ...
      <el-button class="filter-item" v-permission="['admin']" style="margin-left: 10px;" @click="handleCreate"
                 type="primary" icon="el-icon-edit">{{$t('case.add')}}
      </el-button>
    </div>
    <el-table :key='tableKey' :data="list" v-loading="listLoading" border fit highlight-current-row
      ...
      <el-table-column align="center" :label="$t('case.startTime')">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="$t('case.actions')" width="230" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="handleDiagram(scope.row.processInstanceId,scope.row.endTime)">
            Status
          </el-button>
        </template>
      </el-table-column>
          ...
          <el-col :span="12" class="form-item">
            <el-form-item :label="$t('case.processDefinition')">
              <el-select class="filter-item" v-model="temp.processDefinitionId" placeholder="Please select">
                <el-option v-for="item in  processDefiitons" :key="item.id" :label="item.key"
                           :value="item.id">
                </el-option>
              </el-select>
            </el-form-item>
    ...
    <el-dialog title="Process Status" :visible.sync="dialogDiagramVisible" width="80%">
      <iframe :src="dialogDiagramUrl" id="dialogDiagramIframe" frameborder="no" border="0" marginwidth="0"
              marginheight="0"/>
    </el-dialog>
...
<script>
  import { fetchCaseHistoryList, createCase } from '@/api/case'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        fetchCaseHistoryList(this.listQuery).then(response => {
      ...
      handleDiagram(processInstanceId, endTime) {
        this.dialogDiagramVisible = true
        if (endTime) {
          this.dialogDiagramUrl = '/static/flow/modeler/display/displaymodel-embed.html?historyId=' + processInstanceId + '&modelType=runtime&token=' + getToken()
        } else {
          this.dialogDiagramUrl = '/static/flow/modeler/display/displaymodel-embed.html?modelId=' + processInstanceId + '&modelType=runtime&token=' + getToken()
        }
      }
    }
  }
